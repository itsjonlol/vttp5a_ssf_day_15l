...
public class ListRepo {
    ...
    public void leftPush(String key, String value) { //lpush cart apple
        template.opsForList().leftPush(key, value); //template.opsForList().leftpush("cart","apple");
    }
    public void rightPush(String key, String value) {
        template.opsForList().rightPush(key, value);
    }
    //can pop by count or by key. // can i get the string directly?
    public void leftPop(String key) { //lpop cart
        template.opsForList().leftPop(key,1); //template.opsForList().leftpop("cart");
    }
    public void rightPop(String key) {
        template.opsForList().leftPop(key,1);
    }

    public String get(String key, Integer index) { //lindex cart 1
        return template.opsForList().index(key,index).toString(); // String item = tempalte.opsForList().index("cart",1L);

    }
    
    public Long size(String key) { //llen cart
        return template.opsForList().size(key); //long cartLen = template.opsForlist().size("cart");
    }
    //key is the list
